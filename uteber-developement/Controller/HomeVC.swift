...
class HomeVC: UIViewController, Alertable{
    ...
    let revealingSplashView = RevealingSplashView(iconImage: UIImage(named: "logo_main")!, iconInitialSize: CGSize(width: 200, height: 100), backgroundColor: UIColor.white)
    ...
    revealingSplashView.startAnimation()
        ...
        UpdateService.instance.observeTrips { (tripDict) in
            if let tripDict = tripDict {
                ...
                let accptanceStatus = tripDict[TRIP_IS_ACCEPTED] as! Bool
                
                if accptanceStatus == false {
...
extension HomeVC: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, rendererFor overlay: MKOverlay) -> MKOverlayRenderer {
        ...
        lineRenderer.strokeColor = UIColor(red: 255/255, green: 193/255, blue: 7/255, alpha: 0.75)
        lineRenderer.lineWidth = 3
        
        shouldPresentLoadingView(false)
        
        //zoom(toFitAnnotationsForMapView: self.mapView)
    ...
    func performSearch(){
        ...
        if destinationTextField.text == "" {
            showAlert("Please Enter Destination")
            return
...
extension HomeVC: UITextFieldDelegate{
    ...
    func textFieldShouldClear(_ textField: UITextField) -> Bool {
        ...
        DataService.instance.REF_USERS.child(Auth.auth().currentUser!.uid).child(TRIP_COORDINATE).removeValue()
    ...
    func animateTableView(shouldShow: Bool){
        if shouldShow {
            ...
            UIView.animate(withDuration: 0.2, animations:{
                self.tableView.frame = CGRect(x: 20, y: self.view.frame.height, width: self.view.frame.width - 40, height: self.view.frame.height - 170) }, completion: { (finished) in
